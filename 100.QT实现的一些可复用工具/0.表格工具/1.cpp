	...
	connect(removeCell, &QPushButton::clicked, this, [=]()
		{
			QString stagestring;
			QString cmdstring;
			if (!rightTable->item(row, column) || rightTable->item(row, column)->text().isEmpty())
			{
				stagestring = "";
			}
			else
			{
				stagestring = rightTable->item(row, column)->text();
			}
			cmdstring = rightTable->item(row, column)->text();
			if (displayMsgInfo(QString("警告"), QString("是否确定删除项:%1").arg(column == 2 ? QString("口令:") + cmdstring : QString("阶段:") + stagestring), QIcon(":/ControlMonitor/images/C3I-64_64.ico"), QMessageBox::Warning))
			{
				return;
			}
	...
	connect(frontInsertCell, &QPushButton::clicked, this, [=]() {
		...
		{//插入阶段
			while (tmpHeader->next)
			{
				if (tmpHeader->next->startRowNum == row)
				{
					...
					newStageNode->sortNum = tmpHeader->next->sortNum;
					newStageNode->rowSpanCount = 1;
					newStageNode->startRowNum = tmpHeader->next->startRowNum;
					newStageNode->next = tmpHeader->next;
					tmpHeader->next = newStageNode;
		...
		{//插入口令 
			while (tmpHeader->next)
			{
				if (row >= tmpHeader->next->startRowNum && row < (tmpHeader->next->startRowNum + tmpHeader->next->rowSpanCount))
				{
					QString itemInfo = rightTable->item(row, 1)->text();
					addRow(row);
					if (row == tmpHeader->next->startRowNum)
					{
						rightTable->item(row, 1)->setText(itemInfo);
					}
					tmpHeader->next->rowSpanCount += 1;
