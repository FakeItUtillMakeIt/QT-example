#include "UI/controlmonitor.h"
#include "UI/ControlMonitorLogin.h"
...
int main(int argc, char *argv[])
{
	...
	if (!app.isRunning())
	{
		...
		ControlMonitor w;
		w.hide();
		ControlMonitorLogin login(&app, &w);
		login.show();
		return app.exec();
	}
	return 0;
	...
	if (process.waitForFinished()) {
		...
		if (result.count("ControlMonitor", Qt::CaseInsensitive) >= 2) {
			QMessageBox::critical(qApp->activeWindow(), QObject::tr("警告"), QObject::tr("C3I监控软件已启动!"));
			app.exit();
			return 0;
		}
	}
