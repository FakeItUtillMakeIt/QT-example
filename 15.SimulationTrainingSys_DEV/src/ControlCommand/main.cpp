#include "UI/controlcommand.h"
#include "UI/ControlCommandLogin.h"
...
int main(int argc, char *argv[])
{
	...
	if (process.waitForFinished()) {
		...
		if (result.count("ControlCommand", Qt::CaseInsensitive) >= 2) {
			QMessageBox::critical(qApp->activeWindow(), QObject::tr("警告"), QObject::tr("主控软件已启动!"));
			app.exit();
			return 0;
		}
	}
