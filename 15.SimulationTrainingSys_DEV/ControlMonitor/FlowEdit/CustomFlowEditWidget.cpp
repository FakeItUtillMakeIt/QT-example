...
CustomFlowEditWidget* CustomFlowEditWidget::instance = nullptr;
	...
	: QWidget(parent)
{
	...
	leftSearchButton->setIcon(QIcon(":/ControlMonitor/images/Flow/find.png"));
	leftAddFlowButton = new QPushButton;
	leftAddFlowButton->setIcon(QIcon(":/ControlMonitor/images/Flow/add.png"));
	...
	leftProjectFlowList->setMaximumWidth(140);
	leftProjectFlowList->setStyleSheet("\
		...
		");
	leftProjectFlowList->setContentsMargins(0, 0, 0, 0);
	leftProjectFlowList->setLayoutDirection(Qt::LeftToRight);
	leftProjectFlowList->horizontalScrollBar()->hide();
	leftProjectFlowList->verticalScrollBar()->hide();
	leftProjectFlowList->setFocusPolicy(Qt::NoFocus);
	...
	rightFlowUpdateButton->setIcon(QIcon(":/ControlMonitor/images/Flow/update.png"));
	...
	rightFlowDeleteButton->hide();
	rightFlowEditButton->hide();
...
void CustomFlowEditWidget::addNewFlow() {
	...
	lineEdit->setValidator(new QRegExpValidator(QRegExp("\\S+")));
	...
	connect(okButton, &QPushButton::clicked, this, [=]() {
		...
		QListWidgetItem* item1 = new QListWidgetItem(lineEdit->text());
		item1->setTextAlignment(Qt::AlignCenter);
		item1->setIcon(QIcon(QString(":/ControlMonitor/images/Flow/squareBl.png")));
		item1->setSizeHint(QSize(leftProjectFlowList->width(), 40));
		leftProjectFlowList->addItem(item1);
		newFlowName->deleteLater();
...
void CustomFlowEditWidget::loadFlowTable() {
	...
	rightFlowNameLabel->setText(leftProjectFlowList->currentItem()->text() + QString::fromLocal8Bit("-发射流程"));

	for (int i = 0; i < leftProjectFlowList->count(); i++)
	{
		leftProjectFlowList->item(i)->setIcon(QIcon(":/ControlMonitor/images/Flow/squareBl.png"));
	}
	leftProjectFlowList->currentItem()->setIcon(QIcon(":/ControlMonitor/images/Flow/squareWl.png"));
	...
	if (rightFlowTable->item(0, column)->text().contains(tableColumnKey[column]))
	{
		...
		connect(frontInsertCell, &QPushButton::clicked, this, [=]() {
			...
			else
			{
				...
				{
					subTable->setRowHeight(i, rowHeight * rowMaxRowCount[row] / 4);
...
void CustomFlowEditWidget::okButtonClick() {
	...
	if (rightFlowTable->rowCount() == 0)
	{
		qDebug() << QString::fromLocal8Bit("流程为空信息");
		this->deleteLater();
		return;
	}
