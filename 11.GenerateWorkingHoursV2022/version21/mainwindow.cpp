...
void MainWindow::initConnect(){
    ...
    /*年度*/
    connect(ui->btnYearDataClc,&QPushButton::clicked,this,&MainWindow::projectYeaRDataCalcPage);
...
/*年度数据汇总*/
void MainWindow::projectYeaRDataCalcPage(){
    currentPage=CURRENT_PAGE::PROJECT_YEAR_DATA_CALC;
    ...
    pageButtonStateChange(CURRENT_PAGE::PROJECT_YEAR_DATA_CALC);
...
void MainWindow::pageButtonStateChange(CURRENT_PAGE page){
    ...
    ui->btnDataClc->setFlat(false);
    ui->btnYearDataClc->setFlat(false);
    ...
    ui->btnDataClc->setDown(false);
    ui->btnYearDataClc->setDown(false);
    ...
    ui->btnYearDataClc->setStyleSheet("color:rgb(0,0,0)");
    ...
    switch (page) {
        ...
        break;
        case CURRENT_PAGE::PROJECT_YEAR_DATA_CALC:
        ui->btnYearDataClc->setFlat(true);
        ui->btnYearDataClc->setDown(true);
        ui->btnYearDataClc->setStyleSheet("color:rgb(30,144,255)");
        break;
    }
...
/*月度汇总*/
 /***计算汇总***/
void MainWindow::selectMonthMutliFile(){
...
/*年度汇总*/
 /***计算汇总***/
void MainWindow::selectYearMutliFile(){
    QFileDialog* dialog=new QFileDialog(this);
    ...
    ui->tableWidget->clearContents();
    ui->tableWidget->setColumnCount(9+1);

    /*记录工号与项目和月份的关系*/
    //一个工号对应多个项目
    //一个项目对应多个月份和占比
    QMap<QString,QMap<QString,QVector<prj2DateRatio1>>> workNum2PrjMonth;
    /**/
    QMap<QString,int> workNumCount;
    ...
    bool first=true;
        ...
        QStringList firstRowContent;
        for (QString filename:selectFile) {
            QMap<QString,int> workNumInFile;
            QXlsx::Document* excelFile=new QXlsx::Document(filename);
            ...
            for(int i=0;i<rowC;i++){
                ...
                //工号--项目--月--zhanbi
                QVector<prj2DateRatio1> prj2DateRatioV;
                if(workNum2PrjMonth[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))][excelFile->read(i+2, prjIndex).toString()].size()){
                    prj2DateRatioV=workNum2PrjMonth[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))][excelFile->read(i+2, prjIndex).toString()];
                ...
                /**************************************************/
                if(!excelFile->read(i+2,1).toString().isNull() || !excelFile->read(i+2,1).toString().isEmpty()){
                    workNum2PrjMonth[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))][excelFile->read(i+2, prjIndex).toString()]=prj2DateRatioV;
                    //workNum2PrjMonth[QString::number(excelFile->read(i+2,1).toString().toInt())][excelFile->read(i+2, prjIndex).toString()]=prj2DateRatioV;
                    workNumInFile[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))]=1;
                }
                /*************************************************/

                if(!workNum2Name[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))].size()){
                    workNum2Name[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))]=excelFile->read(i+2,2).toString();
                }
                if(!workNum2Career[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))].size()){
                    workNum2Career[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))]=excelFile->read(i+2,3).toString();
                }
                if(!workNum2Department[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))].size()){
                    workNum2Department[QString("%1").arg(excelFile->read(i+2,1).toString().toInt(),3,10,QChar('0'))]=excelFile->read(i+2,4).toString();
                }
                if(!prj2FirstDepart[excelFile->read(i+2,7).toString()].size()){
                    prj2FirstDepart[excelFile->read(i+2,7).toString()]=excelFile->read(i+2,5).toString();
                }
            ...
            for (auto mapEle=workNumInFile.begin();mapEle!=workNumInFile.end();mapEle++) {
                if(mapEle.value()==1){
                    workNumCount[mapEle.key()]+=1;
                }
            }
        }
        ...
        ui->tableWidget->setItem(rowC-1,9+1,new QTableWidgetItem("占比"));
        qDebug()<<rowC<<firstRowContent;
        columnC=0;
        //项目vector
        auto ele=workNum2PrjMonth.constBegin();
        //工号
        while(ele!=workNum2PrjMonth.constEnd()){
            if(ele.key().isNull()){
                ele++;
                continue;
            }
            ...
            while(ssubEle!=subEle.constEnd()){
                ...
                float tmpRatioData=qRound((float)(10000*totalRatio)/(float)workNumCount[ele.key()])/float(10000);
                restRatio-=tmpRatioData;
                ui->tableWidget->setItem(rowC,9,new QTableWidgetItem(QString::number(tmpRatioData)));
                //ui->tableWidget->setItem(rowC,9,new QTableWidgetItem(QString::number(totalRatio)));
                ui->tableWidget->setItem(rowC,1,new QTableWidgetItem(workNum2Name[ele.key()]));
                ui->tableWidget->setItem(rowC,2,new QTableWidgetItem(workNum2Career[ele.key()]));
                ui->tableWidget->setItem(rowC,3,new QTableWidgetItem(workNum2Department[ele.key()]));
                ...
                ui->tableWidget->setItem(rowC,7,new QTableWidgetItem(prj2Type[ssubEle.key()]));
                ui->tableWidget->setItem(rowC,8,new QTableWidgetItem(prj2Attribute[ssubEle.key()]));
                totalRatioV.push_back(tmpRatioData);
                qDebug()<<totalRatio<<"V:"<<totalRatioV;
            ...
            int maxRatioIndex=0;
            float maxRatio=0;
            for(int ratioIndex=0;ratioIndex<totalRatioV.size();ratioIndex++){
                if(maxRatio<totalRatioV[ratioIndex]){
                    maxRatioIndex=ratioIndex;
                    maxRatio=totalRatioV[ratioIndex];
                }
            }
            float endVal=qRound((restRatio+ui->tableWidget->item(rowC-(totalRatioV.size())+maxRatioIndex,9)->text().toFloat())*100000)/float(100000);
            ui->tableWidget->setItem(rowC-(totalRatioV.size())+maxRatioIndex,9,new QTableWidgetItem(QString::number(endVal)));
